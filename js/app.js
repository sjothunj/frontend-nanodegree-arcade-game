...
Player.prototype.update = function() {
    ...
    if (this.y == 0) { // the player made it across so we'll set the reset flag and pause for a split second
        player.reset();
    } else {
        for (var i = 0; i < maxEnemies; i++) { // check each Enemy
			var currentEnemy = allEnemies[i].y + 20; // there is a 20 pixel offset between the y coord of enemy and player
            if (this.y == currentEnemy) { //if the player and the enemy are currently on the same row then
			    if (this.x < allEnemies[i].x+70){ // the right hand side of the image collides before the y coord
					player.reset();
				}
            }
        }
